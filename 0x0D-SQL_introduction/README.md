...
Persistence of data is very crucial to software developer. Also,data should be store in such a way that it can be retrieve at any given time. This helps greatly in managing an enterprise be it small or big. Database storage because of its "ACID" properties helps in the efficient storage and management of data.
This project open my eyes to the concept of a relational database and how it is efficiently managed using a Relational Database Management System(RDMS) like MySQL pronounced as "My Ess Que Ell", postgressql, oracle etc. I used Structured Query Language(SQL) pronounced "sequel", a standardized language for defining the structure of a database(DDL) and manipulate the data therein(DML) to accomplished tasks encompassing queries, subqueries and functions.
